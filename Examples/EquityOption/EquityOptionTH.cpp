...
// std::ostream& operator<<(std::ostream& os, const QuantLib::VanillaOption& option) {
//   os << "VanillaOption\n"
//      << "\tUnderlying: " << option.underlyingPrice() << "\n"
//      << "\tStrike: " << option.strike() << "\n"
//      << "\tExpiry: " << option.exercise()->lastDate() << "\n"
//      << "\tOption Type: " << (option.type() == QuantLib::Option::Call ? "Call" : "Put") << "\n"
...
int main() {
  ...
  try {
    ...
    std::cout << "OPTION CONTRACT PARAMETERS" << std::endl;
    Option::Type type(Option::Put);           std::cout << "Option Type: " << type << std::endl;
    Real underlying = 36;                     std::cout << "Stock Price: " << underlying << std::endl;
    Real strike = 40;                         std::cout << "Strike Price: " << strike << std::endl;
    Spread dividendYield = 0.00;              std::cout << "Dividend Yield: " << dividendYield << " (" << io::rate(dividendYield) << ")" << std::endl; //  std::cout << "DivYield: " << io::rate(dividendYield) << std::endl;
    Rate riskFreeRate = 0.06;                 std::cout << "Risk Free Rate: " << riskFreeRate << " (" << io::rate(riskFreeRate) << ")" << std::endl; //   std::cout << "RiskyRate: " << io::rate(riskFreeRate) << std::endl;
    Volatility volatility = 0.20;             std::cout << "Volatility: " << volatility << " (" << io::volatility(volatility) << ")" << std::endl;         // std::cout << "Volly: " << io::volatility(volatility) << std::endl;
    Date maturity(17, May, 1999);             std::cout << "Expiry: " << maturity << std::endl;
    std::cout << "--------------------------------" << std::endl;
  ...
  auto bsmProcess = ext::make_shared<BlackScholesMertonProcess>(underlyingH, flatDividendTS, flatTermStructure, flatVolTS);
  ...
  //
  // Analytic Options Pricing Models
  //
  ...
  std::cout << std::setw(widths[0]) << std::left << method << std::fixed      // method
            << std::setw(widths[1]) << std::left << europeanOption.NPV()      // European Price
            << std::setw(widths[2]) << std::left << "N/A"                     // Bermudan Price
            << std::setw(widths[3]) << std::left << "N/A"                     // American Price
            << std::endl;
  ...
  Real b = 0.30;
  Real sigma_r = 0.15;
  Real riskPremium = 0.00;
  Real correlation = 0.50;
